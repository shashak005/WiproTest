...
class ViewController: UIViewController,UITableViewDelegate, UITableViewDataSource {
    ...
    func fetchDataFromUrl() {
        let estr = "https://dl.dropboxusercontent.com/s/2iodh4vg0eortkl/facts.json"
            ServiceLayer().performRequestWithServiceNameForGet(serviceURL: "\(estr)", success: { (response,titlestr) in
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if canadaDataModal[indexPath.row].descriptionValue != "" {
            cell!.labelDescription.frame = CGRect(x: 90, y: 25, width: displayWidth - 90, height: heightOFText + 30)
        }
        
        else{
           cell!.labelDescription.frame = CGRect(x: 90, y: 25, width: displayWidth - 90, height: 80)
